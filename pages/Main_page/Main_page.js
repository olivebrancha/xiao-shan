...
Page({
  ...
  data: {
    // 加入贴吧navbar改为 navbar: ['交易市场', '畅所欲言'],
    navbar: ['交易市场'],//导航标题
    currentTab: 0,//导航标题默认索引为0，即显示第一个
    SellDataPostArry: [],//寻物中，只要是关于发帖的全部信息
    SellUserHeadurlArry: [],//寻物用户头像
    SellUsernameArry: [],//寻物用户名称
    SellUpArray: [],//寻物的人点赞
    DataPostArry: [],//帖子
    UserHeadurlArry: [],//用户头像
    UpArray: [],//点赞
    UsernameArry: [],//用户名字
  ...
  wanttobuy:function(e){
    ...
    if (this.data.SellUpArray[buyind] == 0)//说明没点赞过
    {
      ...
      const db = wx.cloud.database({ env: 'cloud1-5gvu20nlcb2d549c' })
   ...
   let that = this
    wx.getStorage({
      ...
      success(res) {
        ...
        ////
        var db = wx.cloud.database()//{ env: 'cloud1-5gvu20nlcb2d549c' }
        let userid = res.data;
    ...
    const get_inf_db = wx.cloud.database()//{ env: 'cloud1-5gvu20nlcb2d549c' }
...
get_Sell_DBinf:function(){
  ...
  const get_Sell_inf_db = wx.cloud.database()//{ env: 'cloud1-5gvu20nlcb2d549c' }
